# cei6/indexers/news_releases_indexer.py
...
def _parse_listing(soup: BeautifulSoup) -> List[ListingItem]:
    ...
    for card in cards:
        title, href = extract_title_and_url(card, "/news_releases/")
        if not href:
            continue
        url = absolutize(LISTING_URL, href)
        issue = extract_issue(card)
        authors = extract_authors(card)  # many releases have no byline; that's ok
